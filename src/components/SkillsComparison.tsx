...
export function SkillsComparisonExact({ 
  ...
  const handleDownloadPDF = () => {
    if (onExportPdf) {
      ...
      // Extract the job description from the first parameter description for the PDF
      const jobDescriptionText = "Job Requirements: " + 
        evaluationParameters.map(p => p.description).join(" ");
      
      // Generate PDF
      exportReportAsPdf(jobDescriptionText, evaluationParameters, candidateEvaluations);
    }
  };
